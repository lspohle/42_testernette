...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/29 08:25:12 by lspohle           #+#    #+#             */
/*   Updated: 2023/01/06 19:51:44 by lspohle          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    test_headline("ft_putstr_fd.c");

    int     test = 1;

    int     fd = open("../txt_files_libft/ft_putstr_fd_test.txt", O_RDWR);
    FILE    *fileCheck;
    FILE    *fileTest;

    //Calling function with various input
    ft_putstr_fd("42 Wolfsburg", fd); // basic input
    write(fd, "\n", 1);
    ft_putstr_fd("\tHello World\t", fd); // whitespaces
    write(fd, "\n", 1);
    ft_putstr_fd("0123456789", fd); // basic input
    write(fd, "\n", 1);
    ft_putstr_fd("\eTest\t", fd); // empty space

    // Opening check file and test file
    fileCheck = fopen("../txt_files_libft/ft_putstr_fd_check.txt", "r");
    fileTest = fopen("../txt_files_libft/ft_putstr_fd_test.txt", "r");
    ...
    // Closing test file (which was open in order to apply ft_putstr_fd())
    close(fd);
}
